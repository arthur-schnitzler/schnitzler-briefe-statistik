<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:for-each
            ...
            <xsl:result-document href="../karte3/karte_{$korrespondenz-nummer}.json">
                <xsl:text>{&#10;</xsl:text>
    ...
    <xsl:template match="tei:list">
        <xsl:variable name="correspAction-gesamt" as="node()" select="."/>
        <xsl:for-each
            select="distinct-values(child::tei:item/descendant::tei:correspDesc[tei:correspAction[@type='received'][1]/tei:persName/@ref='#pmb2121' and tei:correspAction[@type = 'received'][1]/tei:placeName[1][@ref[starts-with(., '#pmb')]]][1]/tei:correspAction[@type = 'sent'][1]/tei:placeName[1][@ref[starts-with(., '#pmb')]])">
            ...
            <xsl:for-each
                ...
                <xsl:value-of
                    select="count($correspAction-gesamt/descendant::tei:correspDesc[tei:correspAction[@type='received'][1]/tei:persName/@ref='#pmb2121'][tei:correspAction[@type = 'sent']/tei:placeName[1] = $from and tei:correspAction[@type = 'received']/tei:placeName[1] = $to])"/>
                <xsl:text>}</xsl:text>

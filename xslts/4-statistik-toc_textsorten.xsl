   ...
   -->
    <xsl:template match="/">
        <xsl:for-each
            ...
            <xsl:result-document indent="no"
                href="../statistik4/statistik_{$korrespondenz-nummer}.csv">
    ...
    <xsl:template match="tei:list">
        ...
        <xsl:for-each select="$textsorten/list/item">
            ...
            <xsl:choose>
                <xsl:when test="$correspAction-gesamt/descendant::tei:objectType[@corresp = $corresp and normalize-space($ana)=''][1]">
                    <xsl:value-of select="."/><xsl:text>,</xsl:text>
                    <xsl:value-of select="count($correspAction-gesamt/descendant::*:objectType[@corresp = $corresp])"/>
                    <xsl:text>&#10;</xsl:text>
                </xsl:when>
                <xsl:when test="$correspAction-gesamt/descendant::tei:objectType[@corresp = $corresp and @ana=$ana][1]">
                    <xsl:value-of select="."/><xsl:text>,</xsl:text>
                    <xsl:value-of select="count($correspAction-gesamt/descendant::*:objectType[@corresp = $corresp and @ana=$ana])"/>
                    <xsl:text>&#10;</xsl:text>
                </xsl:when>
            </xsl:choose>
        </xsl:for-each>
